...
# food_list = """mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
...
for food in food_list.split('\n'):
    ...
    for allergen in allergens:
        if allergen not in possible_foods:
            possible_foods[allergen] = ingredients.copy()
        possible_foods[allergen] = possible_foods[allergen].intersection(ingredients)

all_possible_ingredients_with_allergens = set()
for allergen, foods in possible_foods.items():
    print(f"{allergen}: {foods}")
    all_possible_ingredients_with_allergens = all_possible_ingredients_with_allergens.union(foods)

print(all_possible_ingredients_with_allergens)

count = 0
all_ingredients_count = 0
for food in food_list.split('\n'):
    ingredients = food.split(' (contains ')[0].split(' ')
    all_ingredients_count += len(ingredients)
    for ingredient in ingredients:
        # print(ingredient)
        if ingredient not in all_possible_ingredients_with_allergens:
            count += 1

print(count)

# Part 2
l = [[k, v] for k, v in possible_foods.items()]
...
l = sorted(l)
print(','.join([a[1].pop() for a in sorted(l)]))
