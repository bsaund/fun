...
for entry in passports:
    fields = set([f.split(":")[0] for f in re.split(' |\n', entry) if f])
    count += required_fields.issubset(fields)
print(count)

# Part 2
count = 0
for entry in passports:
    d = {f.split(":")[0]: f.split(":")[1] for f in re.split(' |\n', entry) if f}
    if not required_fields.issubset(set(d)):
        continue
    conditions = [1920 <= int(d["byr"]) <= 2002,
                  2010 <= int(d["iyr"]) <= 2020,
                  2020 <= int(d["eyr"]) <= 2030,
                  any([d["hgt"][-2:] == "cm" and 150 <= int(d["hgt"][0:-2]) <= 193,
                       d["hgt"][-2:] == "in" and 59 <= int(d["hgt"][0:-2]) <= 76]),
                  d["hcl"][0] == "#" and len(d["hcl"]) == 7 and all([c in 'abcdef0123456789' for c in d["hcl"][1:]]),
                  d["ecl"] in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'],
                  len(d['pid']) == 9 and all([c in '0123456789' for c in d['pid']])
                  ]
