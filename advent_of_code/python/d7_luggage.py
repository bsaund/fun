...
for r in rules:
    s = "{color} bags contain {contents_str}"
    m = parse.parse(s, r)
    container = m['color']
    # print(m['color'])
    for c in re.split(" bags?[,.] ?", m['contents_str'])[:-1]:
        if c == "no other":
            continue
        contained_color = ' '.join(c.split(' ')[1:])
        num = int(' '.join(c.split(' ')[0]).replace('no other', '0'))
        contains[container].append((num, contained_color))
        contained_by[contained_color].append(container)
        # print(c)
print(contains)
# print(contained_by)

open_set = set(['shiny gold'])
closed_set = set()
count = 0

while open_set:
    item = open_set.pop()
    count += (item != 'shiny gold')
    if item in closed_set:
        # print(f"{item} not contained by any bag")
        continue
    closed_set.add(item)
    for p in contained_by[item]:
        if p not in closed_set:
            # print(f"{item } contained by {p}")
            open_set.add(p)
print(count)


# Part 2
def number_contained(bag):
    contents = contains[bag]
    if not contents:
        return 0
    return sum([c*(1+number_contained(b)) for c, b in contains[bag]])



print(number_contained("shiny gold"))