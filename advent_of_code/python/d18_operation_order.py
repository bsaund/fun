homework = """3 * 4 * 2 * 5 * (3 + 4 * (6 + 3 + 2 + 8 + 4) + 5 * 7)
2 + 6 * 2 * 4 * (9 + 6 * 7 + 2 * 7) + 3
(9 + 4 * 4 + 5 * 2) * 2 * 7
...
def evaluate(expression: str):
    ...
    for c in expression.split(' '):
        ...
        if c == "(":
            ...
            paren_count -= 1
            if paren_count == 0:
                val = next_op(val, evaluate(in_paren[2:-1]))
                in_paren = ""
                continue
...
def evaluate_p2(exp):
    while '(' in exp:
        cap = get_parens_capture(exp)
        exp = exp.replace(cap, evaluate_p2(cap[2:-2]))


    while '+' in exp:
        exp_split = exp.split(' ')
        ind = exp_split.index('+')
        a = int(exp_split[ind - 1])
        b = int(exp_split[ind + 1])
        exp = exp.replace(f'{a} + {b}', f'{a+b}')
