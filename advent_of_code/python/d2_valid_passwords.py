...
"""
count = 0
parsed_pw_list = [entry for entry in pw_list.split('\n') if entry != ""]
for entry in parsed_pw_list:
    policy, pw = entry.split(': ')
    r, letter = policy.split(' ')
    lower, upper = [int(v) for v in r.split('-')]
    count += (upper >= pw.count(letter) >= lower)

print(count)
format_str = '{lower:d}-{upper:d} {letter}: {pw}'
parsed = [parse.parse(format_str, s).named for s in pw_list.split('\n') if s]
print(len([p['pw'] for p in parsed if p['lower'] <= p['pw'].count(p['letter']) <= p['upper']]))
print(parsed[0])
print(len([e['pw'] for e in [parse.parse(format_str, s) for s in pw_list.split('\n') if s] if int(e[0]) <= e[3].count(e[2]) <= int(e[1])]))
...
for entry in parsed_pw_list:
    ...
    r, letter = policy.split(' ')
    lower, upper = [int(v) for v in r.split('-')]
    count += (pw[lower - 1] == letter) ^ (pw[upper - 1] == letter)
print(count)